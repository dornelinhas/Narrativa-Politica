<template>
  <div v-if="post" class="article-page">
    ...
    <div class="art-container">
      ...
      <!-- META BAR (STATIC - ORIGINAL LOCATION) -->
      <div class="art-meta">
...
/* ── HERO IMAGE ─────────────────────────────── */
.art-hero { position: relative; width: 100%; max-height: 520px; overflow: hidden; background: #000; }
...
.art-body :deep(img) { width: 100%; border-radius: 8px; margin: 2rem 0; border: 2px solid #1C1C1C; }

/* ── REFERENCES (COLLAPSIBLE) ────────────────── */
.art-references { background: var(--color-bg, #F7F7F5); border: 2px solid #1C1C1C; border-radius: 12px; padding: 30px; margin: 20px 0 40px; box-shadow: 6px 6px 0px #1C1C1C; }
.art-references__title { font-family: var(--font-display, 'Archivo Black', sans-serif); font-weight: 900; font-size: .9rem; text-transform: uppercase; letter-spacing: .1em; color: var(--color-dark); margin: 0; border-bottom: 3px solid #DF2028; display: inline-block; padding-bottom: 4px; }
.art-references__content { font-family: var(--font-sans); font-size: .9rem; line-height: 1.7; color: #555; margin-top: 20px; }
